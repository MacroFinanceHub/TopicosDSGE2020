...
// Costo de ajuste del portafolio.
...
model;
    ...
    d = (1+exp(r(-1)))*d(-1)- exp(y)+exp(c)+exp(i)+(phi/2)*(exp(k(+1))-exp(k))^2+psi_3*(d-d_bar)^2;
    ...
    //10. Eq. (13), Tasa de interes domestica
    exp(r) = r_bar;
    //11. Funcion de utilidad
    util=(((exp(c)-omega^(-1)*exp(h)^omega)^(1-gamma))-1)/(1-gamma);
    //12. p. 169, Definicion del ratio Balanza Comercial/PBI
    tb_y = 1-((exp(c)+exp(i)+(phi/2)*(exp(k(+1))-exp(k))^2)/exp(y));
    //13. p. 169, Definicion del ratio Cuenta Corriente/PBI
    ca_y = (1/exp(y))*(d(-1)-d);                                   
end;
